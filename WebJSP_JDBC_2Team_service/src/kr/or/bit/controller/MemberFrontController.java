...
import kr.or.bit.service.DeleteEmpService;
import kr.or.bit.service.Detail;
import kr.or.bit.service.Edit;
import kr.or.bit.service.EditEmpService;
...
import kr.or.bit.service.ShowEmpListService;
import kr.or.bit.service.Update;
import kr.or.bit.service.UpdateEmpService;
...
public class MemberFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		} else if (url_Command.equals("/ShowEmpList.member")) {
			action = new ShowEmpListService();
			forward = action.execute(request, response);

		} else if (url_Command.equals("/EditEmp.member")) {
			action = new EditEmpService();
			forward = action.execute(request, response);

		} else if (url_Command.equals("/UpdateEmp.member")) {
			action = new UpdateEmpService();
			forward = action.execute(request, response);

		} else if (url_Command.equals("/DeleteEmp.member")) {
			action = new DeleteEmpService();
			forward = action.execute(request, response);

		} else if (url_Command.equals("/upload.member")) {
			action = new EmpFileUploadService();
			forward = action.execute(request, response);
			
		} else if (url_Command.equals("/ChartEmp.member")) {
			forward = new ActionForward();
			forward.setRedirect(false);
